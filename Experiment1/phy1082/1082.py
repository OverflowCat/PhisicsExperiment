...
class Spectrometer:
    ...
    PREVIEW_FILENAME = "E:\基物实验程序\PhysicsExperiment\Experiment1\phy1082\Preview.pdf"  # 预习报告模板文件的名称
    DATA_SHEET_FILENAME = "E:\基物实验程序\PhysicsExperiment\Experiment1\phy1082\data.xlsx"  # 数据填写表格的名称
    REPORT_TEMPLATE_FILENAME = "E:\基物实验程序\PhysicsExperiment\Experiment1\phy1082\1082_empty.docx"  # 实验报告模板（未填数据）的名称
    REPORT_OUTPUT_FILENAME = "E:\基物实验程序\PhysicsExperiment\Experiment1\phy1082\1082Report.docx"  # 最后生成实验报告的相对路径

    def __init__(self):
        self.data = {}  # 存放实验中的各个物理量
        self.uncertainty = {}  # 存放物理量的不确定度
        self.report_data = {}  # 存放需要填入实验报告的
        print("1082 钠光双棱镜干涉、钠光劳埃镜干涉 \n1. 实验预习\n2. 数据处理")
        while True:
            try:
    ...
    def calc_data_detail(data_list):
        ...
        a = sqrt(data_list['b'] * data_list['b1'])
        ...
        result_data['b'] = data_list['b']
        result_data['b1'] = data_list['b1']
        result_data['a'] = a
        result_data['S'] = S
        result_data['S1'] = S1
        result_data['D'] = D
   ...
   #     print("%f" % result_data['result_lambda'])
        return result_data
